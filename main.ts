...
function getChapters ( manifest ): number {
	...
	for ( const key in manifest ) {
		if ( Number.parseInt ( key.replace ( "html", "" ) ) > greatest ) greatest = Number.parseInt (
			key.replace ( "html", "" ) );
	}
	return greatest;
}
...
function parseContentChapter ( html: string, id: string, loc: number ): EpubContentChapter | EpubRawChapter {
	...
	return {
		id, loc, html, title, content, summary, afterword, type: "content",
	};
}
...
function parseEpub ( location: string ): Promise<Epub> {
	...
	epub.on ( "error", function () {
		console.error ( `\u001b[38;2;255;50;50mERROR: Invalid file ${ location }\u001b[0m` );
	} );
...
function authPassword ( username: string, password: string ): boolean {
	if ( username == "alex" && password == "Crypto123" ) return true;
...
app.post ( "/upload", async ( req: IncomingMessage, res: ServerResponse ) => {
	...
	form.parse ( req, function ( error, fields, file ) {
		...
		if ( key !== "" && permaUserKeys[ key ].expires > Date.now () ) console.log (
			`\u001b[93m${ permaUserKeys[ key ].user }\u001b[0m upload "${ file.file.originalFilename }" \u001b[38;2;50;255;50mSUCCESSFUL\u001b[0m` ); else console.log (
			`\u001b[93mGUEST\u001b[0m upload "${ file.file.originalFilename }" \u001b[38;2;50;255;50mSUCCESSFUL\u001b[0m` );
	} );
} );
...
io.on ( "connection", async ( socket ) => {
	...
	socket.on ( "key", async ( key ) => {
		...
		if ( userKey === undefined ) {
			console.log (
				`\u001b[93m${ ID }\u001b[0m key auth attempt \u001b[38;2;255;50;50mDENIED - INVALID KEY\u001b[0m` );
			return socket.emit ( "key_invalid" );
		}
		if ( userKey.expires < Date.now () ) {
			console.log (
				`\u001b[93m${ ID }\u001b[0m key auth attempt \u001b[38;2;255;50;50mDENIED - EXPIRED KEY\u001b[0m` );
			return socket.emit ( "key_invalid" );
		}
		console.log (
			`\u001b[93m${ ID }\u001b[0m key auth attempt \u001b[38;2;50;255;50mACCEPTED - USER "${ userKey.user }"\u001b[0m` );
	...
	socket.on ( "username", async ( username ) => {
		if ( !authUsername ( username ) ) {
			console.log (
				`\u001b[93m${ ID }\u001b[0m password auth attempt \u001b[38;2;255;50;50mDENIED - INVALID USERNAME "${ username }"\u001b[0m` );
	...
	socket.on ( "password", async ( password ) => {
		if ( authPassword ( user, password ) ) {
			...
			console.log (
				`\u001b[93m${ ID }\u001b[0m password auth attempt \u001b[38;2;50;255;50mACCEPTED - USER "${ user }"\u001b[0m` );
			sockets[ index ].user = user;
			currentKey = key;
			return socket.emit ( "valid_password", { key, expires } );
		}
		console.log (
			`\u001b[93m${ ID }\u001b[0m password auth attempt \u001b[38;2;255;50;50mDENIED - INVALID PASSWORD "${ password }" FOR USERNAME "${ user }"\u001b[0m` );
		socket.emit ( "invalid_auth" );
		user = "guest";
	} );
	socket.on ( "deauth", async () => {
		console.log (
			`\u001b[92mde-authenticate\u001b[0m - user \u001b[94m"${ user }"\u001b[0m from \u001b[93m${ ID }\u001b[0m` );
		user = "guest";
		if ( currentKey !== undefined ) permaUserKeys[ currentKey ] = undefined;
		currentKey = undefined;
	} );
	socket.on ( "deauthall", async () => {
		for ( let newSocket of sockets ) {
			if ( newSocket.user === user ) newSocket.socket.emit ( "deauth" );
		}
		console.log (
			`\u001b[92mde-authenticate all\u001b[0m - user \u001b[94m"${ user }"\u001b[0m from \u001b[93m${ ID }\u001b[0m` );
	} );
	socket.on ( "list", async () => {
		let time = new Date ().getTime ();
		let results: any[] = [];
		let userArray = [];
		if ( fs.existsSync ( `./Epubs/users/${ user }` ) ) userArray = fs
		.readdirSync ( `./Epubs/users/${ user }` )
		.map ( ( value: string ) => `users/${ user }/${ value }` );
		for ( let book of fs.readdirSync ( "./Epubs" ).concat ( userArray ) ) {
			if ( cache[ book ] !== undefined ) {
				...
				if ( fileExt !== ".epub" ) bookResults = eventHandlers[ fileExt ].parseList (
					`${ __dirname }/Epubs/${ book }` ); else {
		...
		debugLog (
			`\u001b[92mlist\u001b[0m - \u001b[94m${ new Date ().getTime () - time }ms\u001b[0m from \u001b[93m${ ID }\u001b[0m` );
	...
	socket.on ( "parse", async ( book ) => {
		if ( cache[ book ] !== undefined ) {
			...
			debugLog (
				`\u001b[92mparse (NC)\u001b[0m - \u001b[94m${ book }\u001b[0m from \u001b[93m${ ID }\u001b[0m` );
			let results = await parseEpub ( `./Epubs/${ book }` );
			cache[ book ] = results;
			socket.emit ( "results", { book, results } );
		}
	} );
	socket.on ( "delete", async ( book ) => {
		if ( book.startsWith ( "users/" ) && !book.startsWith ( `users/${ user }` ) ) {
			debugLog (
				`\u001b[38;2;255;50;50mdelete\u001b[0m - \u001b[94m${ book }\u001b[0m from \u001b[93m${ ID }\u001b[38;2;255;50;50m DENIED - WRONG USER "${ book.split (
					"/" )[ 1 ] }" EXPECTED "${ user }"\u001b[0m` );
			return;
		}
		debugLog (
			`\u001b[38;2;255;50;50mdelete\u001b[0m - \u001b[94m${ book }\u001b[0m from \u001b[93m${ ID }\u001b[38;2;50;255;50m ACCEPTED\u001b[0m` );
		cache[ book ] = undefined;
		for ( let socketUser of sockets ) {
			if ( book.startsWith ( "users/" ) && socketUser.user === user ) socket.emit ( "delete",
																						  book ); else socket.emit (
				"delete", book );
		}
		fs.rmSync ( `./Epubs/${ book }` );
	} );
	socket.on ( "ao3", async link => {
		try {
			...
			let path = `./Epubs/dl_${ generateKey ( 16 ) }.epub`;
			if ( user !== "guest" ) path = `./Epubs/users/${ user }/dl_${ generateKey ( 16 ) }.epub`;

			const stream = await axios.get ( dlLink, {
				responseType: "stream",
			} );
			...
			for ( let socketUser of sockets ) {
				if ( path.startsWith ( "./Epubs/users/" ) && socketUser.user === user ) socket.emit (
					"update" ); else socket.emit ( "update" );
			}
		} catch ( err ) {
			console.error ( err );
		}
