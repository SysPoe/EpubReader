...
function parseContentChapter(
	html: string,
	id: string,
	loc: number,
): EpubContentChapter | EpubRawChapter {
	const $ = cheerio.load(html);

	let title: string = $("h2").text();
	let summary: string | undefined = $(
		"blockquote.userstuff p.calibre7",
	).text();
...
function parseEpub(location: string): Promise<Epub> {
	...
	epub.on("error", function () {
		console.error(
			`\u001b[38;2;255;50;50mERROR: Invalid file ${location}\u001b[0m`,
		);
	});
	let promise: Promise<Epub> = new Promise((resolve) => {
		epub.on("end", async function () {
			...
			for (let i = 1; i <= chapters; i++) {
				...
				let result = parseContentChapter(
					text,
					`html${i}`,
					chapters - i,
				);
			...
			resolve(
				new Epub(chapters, contentChapters, rawChapters, epub.metadata),
			);
		});
	});
	epub.parse();
...
app.post("/upload", async (req: IncomingMessage, res: ServerResponse) => {
	...
	form.parse(req, function (error, fields, file) {
		...
		let mypath = "./Epubs/";
		if (key !== "" && permaUserKeys[key].expires > Date.now())
			mypath += `users/${permaUserKeys[key].user}/`;
		...
		if (key !== "" && permaUserKeys[key].expires > Date.now())
			console.log(
				`\u001b[93m${permaUserKeys[key].user}\u001b[0m upload "${file.file.originalFilename}" \u001b[38;2;50;255;50mSUCCESSFUL\u001b[0m`,
			);
		else
			console.log(
...
io.on("connection", async (socket) => {
	...
	socket.on("list", async () => {
		...
		let results: any[] = [];
		let userArray = [];
		if (fs.existsSync(`./Epubs/users/${user}`))
			userArray = fs
		...
		for (let book of fs.readdirSync("./Epubs").concat(userArray)) {
			if (cache[book] !== undefined) {
				...
				if (fileExt !== ".epub")
					bookResults = eventHandlers[fileExt].parseList(
						`${__dirname}/Epubs/${book}`,
					);
				else {
	...
	socket.on("parse", async (book) => {
		if (cache[book] !== undefined) {
			...
			debugLog(
				`\u001b[92mparse (C)\u001b[0m - \u001b[94m${book}\u001b[0m from \u001b[93m${ID}\u001b[0m`,
			);
		} else {
			...
			const fileExt = path.extname(book).toLowerCase();
			if (fileExt !== ".epub")
				return parseEmitter.emit(fileExt, {
					loc: `${__dirname}/Epubs/${book}`,
					socket,
					user,
				});
			debugLog(
	...
	socket.on("delete", async (book) => {
		if (book.startsWith("users/") && !book.startsWith(`users/${user}`)) {
			debugLog(
				`\u001b[38;2;255;50;50mdelete\u001b[0m - \u001b[94m${book}\u001b[0m from \u001b[93m${ID}\u001b[38;2;255;50;50m DENIED - WRONG USER "${
					book.split("/")[1]
				}" EXPECTED "${user}"\u001b[0m`,
			);
			return;
